...
public class NetconfSession {
    ...
    private static final String EMPTY_CONFIGURATION_TAG = "";
    ...
    return reply;
        ...
        @VisibleForTesting
        String getRpcReply(String rpc, int messageId) throws IOException {
            // write the rpc to the device
            BufferedReader bufferedReader = getRpcReplyRunning(rpc, messageId);
    ...
    private BufferedReader getRpcReplyRunning(String rpc, int messageId) throws IOException {
        // RFC conformance for XML type, namespaces and message ids for RPCs
        rpc = rpc.replace("<rpc>", "<rpc xmlns=\"" + NetconfConstants.URN_XML_NS_NETCONF_BASE_1_0 + "\" message-id=\"" + messageId + "\">").trim();
        if (!rpc.contains(NetconfConstants.XML_VERSION)) {
            rpc = NetconfConstants.XML_VERSION + rpc;
    ...
    private String getConfig(String target, String configTree, int messageId)
            throws IOException {
        ...
        String rpc = "" + "<rpc>" +
                ...
                "</rpc>" +
                NetconfConstants.DEVICE_PROMPT;
        lastRpcReply = getRpcReply(rpc, messageId);
        return lastRpcReply;
    }
     ...
     */
    public XML executeRPC(String rpcContent, int messageId) throws SAXException, IOException {
        String rpcReply = getRpcReply(fixupRpc(rpcContent), messageId);
     ...
     * @throws org.xml.sax.SAXException If there are errors parsing the XML reply.
     */
    public XML getRunningConfig(String configTree, int messageId) throws SAXException,
            IOException {
        return convertToXML(getConfig(RUNNING_CONFIG, configTree, messageId));
    }
